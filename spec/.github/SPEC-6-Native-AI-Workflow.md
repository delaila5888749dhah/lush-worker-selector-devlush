...
| 2.8 | 2026-04-29 | **Phase 10 dedicated spec doc (audit M1 fix — Option A).** Added `spec/phase10.md` mirroring `blueprint.md §8.1–§8.8` as §10.1–§10.8 (1-to-1), so audits can reach §10.x directly without dereferencing the Sync Matrix in `blueprint.md` §11. Header explicitly states `blueprint.md` is the single source of truth and `phase10.md` mirrors it; each §10.x section back-links to its §8.x counterpart. Zero content changes — documentation traceability only (CHANGE_CLASS=spec_sync). |
| 2.9 | 2026-04-29 | **Phase 10 mirror-fidelity fix.** `spec/phase10.md`: removed broken `#L###` line-number fragments from back-links to `blueprint.md` (rendered Markdown does not resolve those anchors), and aligned §10.1–§10.8 section titles verbatim with the corresponding `blueprint.md §8.x` headings to eliminate paraphrase drift. Documentation only (CHANGE_CLASS=spec_sync). |
