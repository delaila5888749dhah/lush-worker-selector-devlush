...
def handle_ui_lock_focus_shift(worker_id: str) -> bool:
    ...
    """
    driver = _get_driver(worker_id)
    # Pass the GivexDriver wrapper (not the raw Selenium driver) so the
    # driver-level helper can route clicks through ``bounding_box_click``
    # (CDP ``Input.dispatchMouseEvent`` path, isTrusted=True) — Phase 4 [B2].
